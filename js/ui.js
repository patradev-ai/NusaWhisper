...
class UI {
    ...
    updateThemeToggleIcon() {
        ...
        if (themeIcon) {
            themeIcon.className = this.currentTheme === 'light' ? 'fas fa-moon text-gray-600 dark:text-gray-400' : 'fas fa-sun text-gray-600 dark:text-gray-400';
        }
    }
    ...
    updateOnlineStatusDisplay(isOnline) {
        ...
        if (indicator) {
            indicator.className = `w-2 h-2 rounded-full ${isOnline ? 'bg-green-500' : 'bg-gray-400'}`;
        }
        ...
        if (toggle) {
            toggle.className = `relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${isOnline ? 'bg-green-500' : 'bg-gray-300'}`;
    ...
    showUserInfoModal(userInfo) {
        ...
        if (avatar) avatar.textContent = userInfo.address.substring(2, 4).toUpperCase();
        if (nickname) nickname.textContent = userInfo.nickname || Utils.shortenAddress(userInfo.address);
    ...
    updateChatHeader(roomName = 'General', chatType = 'room', memberCount = 0) {
        ...
        if (chatTitle) {
            if (chatType === 'room') {
                chatTitle.textContent = roomName.charAt(0).toUpperCase() + roomName.slice(1);
            } else {
        ...
        if (chatIcon) {
            chatIcon.className = chatType === 'room' ? 'fas fa-hashtag text-white text-sm' : 'fas fa-envelope text-white text-sm';
        }
    ...
    createToastElement(message, type) {
        ...
        toast.className = `toast px-4 py-3 rounded-lg shadow-lg text-white text-sm flex items-center gap-2 ${this.getToastColorClass(type)}`;
    ...
    showConnectionStatus(isConnected) {
        ...
        status.className = `connection-status px-3 py-1 rounded-full text-white text-xs font-medium ${isConnected ? 'connected' : 'disconnected'}`;
