...
class DecentralChat {
    ...
    async init() {
        try {
            ...
            console.log('DecentralChat initialized successfully');
        } catch (error) {
            console.error('Failed to initialize DecentralChat:', error);
    ...
    setupEventListeners() {
        ...
        document.getElementById('toggleOnlineStatus').addEventListener('click', () => {
        ...
        document.getElementById('createRoomBtnMobile').addEventListener('click', () => {
            this.ui.showCreateRoomModal();
        });
        ...
        document.getElementById('joinInviteBtnMobile').addEventListener('click', () => {
            this.ui.showJoinInviteModal();
        });

        document.getElementById('joinByInviteConfirm').addEventListener('click', () => {
            this.handleJoinByInvite();
        });

        document.getElementById('cancelJoinInvite').addEventListener('click', () => {
        ...
        document.getElementById('closeRoomOptionsModal').addEventListener('click', () => {
        ...
        document.getElementById('startDirectChatBtn').addEventListener('click', () => {
            this.handleStartDirectChatFromUserInfo();
        });

        document.getElementById('cancelDirectChat').addEventListener('click', () => {
        ...
        document.getElementById('directChatModal').addEventListener('click', (e) => {
        ...
        document.getElementById('roomOptionsModal').addEventListener('click', (e) => {
    ...
    async checkExistingConnection() {
        try {
            ...
            if (user) {
                ...
                document.getElementById('nicknameInput').value = nickname === user.shortAddress ? '' : nickname;
    ...
    async handleAccountChange(newAccount) {
        if (this.currentUser && this.currentUser.address.toLowerCase() !== newAccount.toLowerCase()) {
    ...
    async handleSaveNickname() {
        try {
            ...
            const success = await this.auth.setUserNickname(this.currentUser.address, nickname);
            if (success) {
    ...
    async handleToggleOnlineStatus() {
        try {
            ...
            this.ui.showToast(`Status set to ${isOnline ? 'online' : 'offline'}`, 'info');
        } catch (error) {
    ...
    async handleGenerateInvite() {
        try {
            const inviteCode = await this.chat.generateRoomInvite(this.chat.currentRoom);
            this.ui.hideRoomOptionsModal();
    ...
    async handleClearChat() {
        try {
            const confirmed = confirm('Are you sure you want to clear all messages in this room? This action cannot be undone.');
            if (!confirmed) return;
    ...
    async handleLeaveRoom() {
        try {
            ...
            const confirmed = confirm(`Are you sure you want to leave the "${this.chat.currentRoom}" room?`);
            if (!confirmed) return;
    ...
    async handleStartDirectChat() {
        try {
            ...
            this.ui.showToast(error.message || 'Failed to start direct chat', 'error');
        }
    }
