...
class Chat {
    ...
    async createRoom(roomName, isPrivate = false) {
        try {
            ...
            await new Promise((resolve, reject) => {
                this.roomsRef.get(roomId).get('info').put(roomData, (ack) => {
    ...
    setupRoomMessageListener() {
        if (this.messagesRef) {
            this.messagesRef.map().on((data, key) => {
                if (data && key && !key.startsWith('~') && this.currentChatType === 'room') {
        ...
        if (this.currentRoomRef) {
            this.currentRoomRef.get('users').map().on((data, key) => {
    ...
    setupDirectMessageListener() {
        if (this.directMessagesRef) {
            this.directMessagesRef.map().on((data, key) => {
                if (data && key && !key.startsWith('~') && this.currentChatType === 'direct') {
                    this.handleNewMessage(data, key);
                }
            });
        }
    ...
    async sendMessage(messageText, user) {
        try {
            ...
            try {
                const signedData = await auth.signChatMessage(messageText, user.address);
    ...
    createMessageElement(message, isOwn) {
        ...
        messageBubble.className = `message-bubble p-3 ${isOwn ? 'message-own text-white' : 'message-other text-gray-800 dark:text-white'}`;
        ...
        messageInfo.className = `message-timestamp flex items-center gap-2 mt-1 ${isOwn ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`;
    ...
    async getRoomInfo(roomName) {
        try {
            return new Promise((resolve) => {
                this.roomsRef.get(roomName).get('info').once((data) => {
                    resolve(data);
                });
            });
        } catch (error) {
    ...
    async generateRoomInvite(roomName) {
        try {
            ...
            await new Promise((resolve, reject) => {
                this.gun.get('invites').get(inviteCode).put(inviteData, (ack) => {
                    if (ack.err) reject(new Error(ack.err));
                    else resolve();
                });
            });
    ...
    async joinByInvite(inviteCode) {
        try {
            const inviteData = await new Promise((resolve) => {
                this.gun.get('invites').get(inviteCode).once((data) => {
                    resolve(data);
                });
            });
    ...
    createUserElement(userInfo) {
        ...
        userDiv.className = 'flex items-center space-x-3 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer';
        userDiv.onclick = () => this.showUserInfo(userInfo);
        
        const avatar = document.createElement('div');
        avatar.className = 'w-8 h-8 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white text-sm font-semibold';
        ...
        nickname.textContent = userInfo.nickname || userInfo.address.substring(0, 8) + '...';
    ...
    async startDirectChat(partnerAddress) {
        try {
            ...
            const chatId = this.generateDirectChatId(this.currentUser.address, partnerAddress);
